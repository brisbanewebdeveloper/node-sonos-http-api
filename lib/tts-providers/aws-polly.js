...
function polly(phrase, voiceName, query) {
  if (!settings.aws) {
  ...
  if (settings.aws.engine) {
    synthesizeParameters.Engine = settings.aws.engine;
  }
  if (voiceName) {
    // logger.info(`Using the voice "${voiceName}" for "${phrase}"`);
    synthesizeParameters.VoiceId = voiceName;
  }
  if (query) {
    if (query.ssml) {
      logger.info(`Using SSML for "${phrase}"`);
      synthesizeParameters.TextType = 'ssml';
      // dynamicParameters.Text = phrase;
    }
  }

  const phraseHash = crypto.createHash('sha1').update(phrase).digest('hex');
  const filename = `polly-${phraseHash}-${synthesizeParameters.Engine}-${synthesizeParameters.VoiceId}.mp3`;
  ...
  try {
    ...
    logger.info(`announce file for phrase "${phrase.trim()}" does not seem to exist, downloading`);
  }
