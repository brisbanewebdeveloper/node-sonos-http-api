...
const PRESETS_FILENAME = `${settings.webroot}/presets.json`;
const presets = {};

function readPresetsFromDir(presetPath) {
  let files;
  try {
  ...
  }).filter(file => !file.stat.isDirectory() && !file.name.startsWith('.') && file.name.endsWith('.json')).forEach((file) => {
...
function readPresetsFromFile(filename) {
  try {
...
function initPresets() {
  ...
  readPresetsFromFile(PRESETS_FILENAME);
  readPresetsFromDir(PRESETS_PATH);
